...
import injectedModule from "@web3-onboard/injected-wallets"; // imported the injectedModule which is necessary to connect to MetaMask

import SendTransaction from "./SendTransaction";

const injected = injectedModule(); // creating an instance

const onboard = Onboard({
    wallets: [injected], // passing the injectedModule to the wallets array
    chains: [
        {
            id: "123456",
            token: "ETH",
...
const Navigation: React.FC = () => {
    ...
    const handleConnect = useCallback(async () => {
        ...
        // add optional chaining to prevent error when metamaskWallet is undefined (occurs on double click of connect wallet button)
        if (
            ...
            <nav className="max-w-[85rem] w-full mx-auto px-4 sm:flex sm:items-center sm:justify-between">
                ...
                <div className="hs-collapse overflow-hidden transition-all duration-300 basis-full grow sm:block">
                    <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-end sm:mt-0 sm:pl-5">
                        {wallet && (
                            <>
                                {/* passing wallet state in send transaction in order to prepopulate sender address input value */}
